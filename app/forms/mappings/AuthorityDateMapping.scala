...
private[forms] object AuthorityDateMapping {
  ...
  private[forms] def mapping(baseKey: Option[String], isStartDateForm: Boolean) = {
    val keyInfix = if (isStartDateForm) startInfix else endInfix

    Forms
      .tuple(dayKey -> dayMapping(baseKey), monthKey -> monthMapping(baseKey), yearKey -> yearMapping(baseKey))
  ...
  private def dayMapping(baseKey: Option[String]): Mapping[String] = AdditionalConstraintsMapping(
    text()
    ...
    Seq(
      ConditionalConstraint(
        isAnyFieldPopulatedCondition(Seq(keyBuilder(baseKey, monthKey), keyBuilder(baseKey, yearKey))),
  ...
  private def monthMapping(baseKey: Option[String]): Mapping[String] = AdditionalConstraintsMapping(
    text()
      .verifying("authorityDate.error.invalid.month", isEmptyOr(isInRange(minValidDayOrMonth, maxValidMonth))),
    Seq(
      ConditionalConstraint(
        isAnyFieldPopulatedCondition(Seq(keyBuilder(baseKey, dayKey), keyBuilder(baseKey, yearKey))),
  ...
  private def yearMapping(baseKey: Option[String]): Mapping[String] = AdditionalConstraintsMapping(
    text()
      .verifying("authorityDate.error.invalid.year", isEmptyOr(isInRange(minAllowedYearValue, maxAllowedYearValue))),
    Seq(
      ConditionalConstraint(
        isAnyFieldPopulatedCondition(Seq(keyBuilder(baseKey, monthKey), keyBuilder(baseKey, dayKey))),
  ...
  private def keyBuilder(baseKey: Option[String], fieldKey: String) =
    baseKey.fold(fieldKey)(baseKey => s"$baseKey.$fieldKey")
}
