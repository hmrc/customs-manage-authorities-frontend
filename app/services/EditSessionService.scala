...
class EditSessionService @Inject()(sessionRepository: SessionRepository,
  ...
  private def populateStartPage(userAnswers: UserAnswers,
                                accountId: String,
                                authorityId: String,
                                authority: StandingAuthority): Future[UserAnswers] = {
    val authorityStart =
      if (authority.authorisedFromDate == dateTimeService.localDate()) AuthorityStart.Today else AuthorityStart.Setdate

    Future.fromTry(
      userAnswers.set(
        EditAuthorityStartPage(accountId, authorityId),
        authorityStart)(AuthorityStart.writes)
    )
  }

  private def populateStartDatePage(userAnswers: UserAnswers,
                                    accountId: String,
                                    authorityId: String,
                                    authority: StandingAuthority): Future[UserAnswers] = {
  ...
  private def populateEndDatePage(userAnswers: UserAnswers,
                                  accountId: String,
                                  authorityId: String,
                                  authority: StandingAuthority): Future[UserAnswers] = {
  ...
  private def populateEndPage(userAnswers: UserAnswers,
                              accountId: String,
                              authorityId: String,
                              authority: StandingAuthority): Future[UserAnswers] = {
  ...
  private def populateShowBalance(userAnswers: UserAnswers,
                                  accountId: String,
                                  authorityId: String,
                                  authority: StandingAuthority): Future[UserAnswers] = {
    Future.fromTry(
      userAnswers.set(EditShowBalancePage(accountId, authorityId),
        ShowBalance.fromBoolean(authority.viewBalance))(ShowBalance.writes)
    )
  }
