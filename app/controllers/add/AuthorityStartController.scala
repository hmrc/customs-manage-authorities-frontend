...
class AuthorityStartController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(view(preparedForm, mode,navigator.backLinkRoute(mode,controllers.routes.AccountsController.onPageLoad(mode))))
  }
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(view(formWithErrors, mode, navigator.backLinkRoute(mode,controllers.routes.AccountsController.onPageLoad(mode))))),
