...
class CheckYourAnswersEditHelper(
  ...
  private def authorityStartRow: Option[SummaryListRow] =
    userAnswers
      .get(EditAuthorityStartPage(accountId, authorityId))
      .flatMap {
  ...
  private def authorityEndRow: Option[SummaryListRow] =
    userAnswers
      .get(EditAuthorityEndPage(accountId, authorityId))
      .flatMap {
  ...
  private def amendValueAttributeForNIEoriAndDefermentAccount(
    accountNumberRowFromHelper: Seq[SummaryListRow],
    accNumber: String
  ): Seq[SummaryListRow] =
    accountNumberRowFromHelper.map { currentRow =>
      if (
        (account.accountType == CdsDutyDefermentAccount) &&
        standingAuthority.authorisedEori.startsWith(nIEORIPrefix)
      ) {
        currentRow.copy(value =
          currentRow.value.copy(content =
            HtmlContent(messages("manageAuthorities.table.heading.account.CdsDutyDefermentAccount.Ni", accNumber))
          )
        )
      } else {
        currentRow
