...
trait Generators extends UserAnswersGenerator with PageGenerators with ModelGenerators with UserAnswersEntryGenerators {
  ...
  def genIntersperseString(
    ...
    } yield seq1.toSeq.zip(seq2).foldRight(emptyString) {
      case ((n, Some(v)), m) =>
        m + n + v
      case ((n, _), m)       =>
        m + n
  ...
  def decimalsBelowValue(value: Double): Gen[Double] =
    arbitrary[Double] suchThat (_ < value)

  def decimalsAboveValue(value: Double): Gen[Double] =
    arbitrary[Double] suchThat (_ > value)

  def decimalsOutsideRange(min: Double, max: Double): Gen[Double] =
