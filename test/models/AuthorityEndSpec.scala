...
class AuthorityEndSpec extends WordSpec with MustMatchers with ScalaCheckPropertyChecks with OptionValues {

  "AuthorityEnd" must {

    "deserialise valid values" in {

      val gen = Gen.oneOf(AuthorityEnd.values.toSeq)
    ...
    "fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!AuthorityEnd.values.map(_.toString).contains(_))
