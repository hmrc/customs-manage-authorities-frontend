...
class EditAuthorityEndControllerSpec extends SpecBase with MockitoSugar {

  private def onwardRoute = Call("GET", "/foo")

  private lazy val authorityEndRoute = controllers.edit.routes.EditAuthorityEndController.onPageLoad("someId", "someId").url

  private val formProvider = new AuthorityEndFormProvider()
  private val form = formProvider()

  "EditAuthorityEnd Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[EditAuthorityEndView]

        status(result) mustEqual OK

        contentAsString(result) mustEqual
          view(form, "someId", "someId")(request, messages(application)).toString
      }
    }

    "populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = UserAnswers(userAnswersId).set(EditAuthorityEndPage("someId", "someId"), AuthorityEnd.values.head)(AuthorityEnd.writes).success.value
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(form.fill(AuthorityEnd.values.head), "someId", "someId")(request, messages(application)).toString
      }
    }
