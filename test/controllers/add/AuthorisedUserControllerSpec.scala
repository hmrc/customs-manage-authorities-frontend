...
class AuthorisedUserControllerSpec extends SpecBase with MockitoSugar {
  ...
  "onSubmit" must {
    ...
    "redirect to next page for valid data when user has selected cash,guarantee and DD accounts " +
      "and input EORI is GB EORI" in new SetUp {
        val userAnswers: UserAnswers = userAnswersTodayToIndefinite
          .set(AccountsPage, List(cashAccount, generalGuarantee, dutyDeferment))
          .success
          .value
  ...
  trait SetUp {
    ...
    val standingAuthority: StandingAuthority =
      StandingAuthority(gbEori, LocalDate.now(), Option(LocalDate.now().plusDays(1)), viewBalance = true)
    ...
    val userAnswersTodayToIndefiniteWithXIEori: UserAnswers =
      UserAnswers("id")
        ...
        .success
        .value
        .set(EoriNumberPage, CompanyDetails(xiEori, Some("companyName")))
        .success
        .value
        .set(AuthorityStartPage, AuthorityStart.Today)(AuthorityStart.writes)
        .success
